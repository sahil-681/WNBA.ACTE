...
metric <- "a40"
season <- "2020"
df.grid <- readRDS(paste0("data/clusteredmodel/cluster1/", metric, season, b2bdata, "GridXOLS102_cluster1.rds"))
      ...
      fluidRow(
        ...
        column(3,
               selectInput(inputId = "season",
                           ...
                           choices = sort(unique(as.character(seq(2018, 2020)))))),
        column(3,
               # Add an actionButton
...
server <- function(input, output, session) {
  ...
  submit_click <- eventReactive(input$submit_button, {
    ...
    if (length(outcomes) > 0) {
      for (po in outcomes) {
        df.grid <- readRDS(paste0("data/model/", po, s, "nob2b", "GridXOLS102.rds"))
  ...
  observe({
    ...
    df.grid_cluster <- readRDS(paste0("data/clusteredmodel/cluster", selected_cluster, "/a40", s, b2bdata, "GridXOLS102_cluster", selected_cluster, ".rds"))
  ...
  cluster_submit_click <- eventReactive(input$cluster_submit_button, {
    ...
    if (length(outcomes) > 0) {
      for (po in outcomes) {
        df.grid <- readRDS(paste0("data/clusteredmodel/cluster", clusters, "/", po, s, b2bdata, "GridXOLS102_cluster", clusters, ".rds"))
        if (i < length(outcomes)) {
          lall[[i]] <- df.grid[, paste0("Pred.", po)]
          i <- i + 1
        }
