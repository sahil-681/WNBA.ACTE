...
for (yname in yvars){
  ...
  # summary(lmod)
  #saveRDS(lmod, file = paste0("output/model/cef/", filename, "_1003.rds"))
  readRDS(paste0("output/model/cef/", filename, "_1003.rds"))
...
ggplot(dg, aes(x=age.f,y=p40,col=trt.f)) + 
  geom_jitter() +
  geom_boxplot(alpha=0.2) + 
  facet_wrap(~trt.f) + 
  scale_y_continuous()

#plot(df$a40, lmod$residuals)
